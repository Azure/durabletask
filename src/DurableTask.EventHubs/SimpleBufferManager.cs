...
namespace DurableTask.EventHubs
{
    ...
    class SimpleBufferManager : IBufferManager
    {
        ...
        public byte[] TakeBuffer(int bufferSize)
        {
            ...
            {
                // To avoid too many buffers of varying sizes, we allocate them in power of 2 sizes
                if (bufferSize <= 1024)
                {
                    bufferSize = 1024;
                }
                ...
                {
                    bufferSize = GetRoundedBufferSize((uint)bufferSize);
                }
            }
        ...
        static int GetRoundedBufferSize(uint v)
        {
            v--;
            v |= v >> 1;
            v |= v >> 2;
