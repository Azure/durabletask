...
namespace DurableTask.EventHubs
{
    ...
    /// </summary>
    public sealed class EventHubsOrchestrationService :
        ...
        IDisposable
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="EventHubsOrchestrationService"/> class.
        /// </summary>
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="EventHubsOrchestrationService"/> class with a custom instance store.
        /// </summary>
        /// <param name="settings">The settings used to configure the orchestration service.</param>
        /// <param name="customInstanceStore">Custom UserDefined Instance store to be used with the EventHubsOrchestrationService</param>
        public EventHubsOrchestrationService(EventHubsOrchestrationServiceSettings settings, IOrchestrationServiceInstanceStore customInstanceStore)
        {
            if (settings == null)
            {
            ...
            EventHubsOrchestrationServiceSettings.Validate(settings);

            this.settings = settings;
            this.stats = new AzureStorageOrchestrationServiceStats();

            this.listener = new EventHubListener(settings, this.stats);
        ...
        async Task EnsureTaskHubAsync()
        {
            ...
            {
                EtwEventSource.Log.GeneralError(
                    this.storageAccountName,
        ...
        async Task GetTaskHubCreatorTask()
        {
            await Task.WhenAll(
                ...
                this.listener.CreateLeaseStoreAsync());

            // work item queue creation
        ...
        public async Task DeleteAsync(bool deleteInstanceStore)
        {
            ...
            tasks.Add(this.listener.DeletePartitionsAsync());
        ...
        public async Task StartAsync()
        {
            ...
            await this.listener.StartAsync();
        ...
        void ReportStats()
        {
            ...
            EtwEventSource.Log.OrchestrationServiceStats(
                this.storageAccountName,
                this.settings.TaskHubName,
                storageRequests,
                messagesSent,
        ...
        internal static Task<CloudQueue[]> GetControlQueuesAsync(
            ...
            int defaultPartitionCount)
        {
            ...
            return Task.FromResult(new CloudQueue[0]);
        }
                ...
                try
                {
                    ...
                    foreach (MessageData message in session.CurrentMessageBatch)
                    {
                        if (session.IsOutOfOrderMessage(message))
                        {
                            ...
                            EtwEventSource.Log.ReceivedOutOfOrderMessage(
                                this.storageAccountName,
                                this.settings.TaskHubName,
                    ...
                    if (!this.IsExecutableInstance(session.RuntimeState, orchestrationWorkItem.NewMessages, out string warningMessage))
                    {
                        ...
                        EtwEventSource.Log.DiscardingWorkItem(
                            this.storageAccountName,
                            this.settings.TaskHubName,
                            session.Instance.InstanceId,
                            session.Instance.ExecutionId,
        ...
        internal static void TraceMessageReceived(MessageData data, string storageAccountName, string taskHubName)
        {
            ...
            EtwEventSource.Log.ReceivedMessage(
                data.ActivityId,
                storageAccountName,
                taskHubName,
        ...
        public async Task CreateTaskOrchestrationAsync(TaskMessage creationMessage, OrchestrationStatus[] dedupeStatuses)
        {
            ...
            MessageSender sender = this.listener.GetOrCreatePartitionSender(
                existingInstance.OrchestrationInstance.InstanceId);

            await sender.SendSingleMessageAsync(creationMessage);
            ...
            // TODO: Large message handling via inputStatusOverride
