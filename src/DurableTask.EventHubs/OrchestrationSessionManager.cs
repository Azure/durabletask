...
namespace DurableTask.EventHubs
{
    ...
    {
        readonly ConcurrentDictionary<string, OrchestrationSession> activeOrchestrationSessions = new ConcurrentDictionary<string, OrchestrationSession>(StringComparer.OrdinalIgnoreCase);
        ...
        public void ReleaseSession(string instanceId, CancellationToken cancellationToken)
        {
            ...
            lock (this.messageAndSessionLock)
            {
                ...
                if (this.activeOrchestrationSessions.TryRemove(instanceId, out OrchestrationSession session))
                {
