...
namespace DurableTask.EventHubs.Partitioning
{
    using System;
    ...
    sealed class PartitionManager<T> where T : Lease
    {
        ...
        async Task LeaseRenewer()
        {
            ...
            while (this.isStarted == 1 || !shutdownComplete)
            {
                ...
                {
                    EtwEventSource.Log.PartitionManagerError(this.accountName, this.taskHub, this.workerName, string.Empty, ex, Utils.ExtensionVersion);
                }
            }
                ...
                if (moreShardsNeeded > 0)
                {
                    ...
                    if (expiredLeases.Count > 0)
                    {
                        foreach (T leaseToTake in expiredLeases)
                        {
                            ...
                            EtwEventSource.Log.LeaseAcquisitionStarted(
                                this.accountName,
                                this.taskHub,
                                this.workerName,
                            ...
                            if (leaseAcquired)
                            {
                                EtwEventSource.Log.LeaseAcquisitionSucceeded(
                                    this.accountName,
                                    this.taskHub,
                                    this.workerName,
                                    leaseToTake.PartitionId,
                        ...
                        if (workerToStealFrom.Value > target - (moreShardsNeeded > 1 ? 1 : 0))
                        {
                            foreach (var kvp in allShards)
                            {
                                if (string.Equals(kvp.Value.Owner, workerToStealFrom.Key, StringComparison.OrdinalIgnoreCase))
                                {
                                    ...
                                    EtwEventSource.Log.AttemptingToStealLease(
                                        this.accountName,
                                        this.taskHub,
                                        this.workerName,
                                    ...
                                    if (leaseStolen)
                                    {
                                        EtwEventSource.Log.LeaseStealingSucceeded(
                                            this.accountName,
                                            this.taskHub,
                                            this.workerName,
                                            workerToStealFrom.Key,
        ...
        async Task<bool> RenewLeaseAsync(T lease)
        {
            ...
            try
            {
                EtwEventSource.Log.StartingLeaseRenewal(
                    this.accountName,
                    this.taskHub,
                    this.workerName,
                    lease.PartitionId,
            ...
            EtwEventSource.Log.LeaseRenewalResult(
                this.accountName,
                this.taskHub,
                this.workerName,
                lease.PartitionId,
            ...
            if (!renewed)
            {
                EtwEventSource.Log.LeaseRenewalFailed(
                    this.accountName,
                    this.taskHub,
                    this.workerName,
                    lease.PartitionId,
        ...
        async Task<bool> AcquireLeaseAsync(T lease)
        {
            ...
            {
                EtwEventSource.Log.LeaseAcquisitionFailed(
                    this.accountName,
                    this.taskHub,
                    this.workerName,
                    lease.PartitionId,
            ...
            catch (Exception ex)
            {
                ...
                EtwEventSource.Log.PartitionManagerError(
                    this.accountName,
                    this.taskHub,
                    this.workerName,
                    lease.PartitionId,
        ...
        async Task<bool> StealLeaseAsync(T lease)
        {
            ...
            catch (LeaseLostException)
            {
                ...
                EtwEventSource.Log.LeaseStealingFailed(this.accountName, this.taskHub, this.workerName, lease.PartitionId, Utils.ExtensionVersion);
            }
            catch (Exception ex)
            {
                // Eat any exceptions during stealing
                EtwEventSource.Log.PartitionManagerError(this.accountName, this.taskHub, this.workerName, lease.PartitionId, ex, Utils.ExtensionVersion);
            }
        ...
        async Task AddLeaseAsync(T lease)
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    EtwEventSource.Log.LeaseRemoved(
                        this.accountName,
                        this.taskHub,
                        this.workerName,
                        lease.PartitionId,
                ...
                catch (LeaseLostException)
                {
                    ...
                    EtwEventSource.Log.LeaseRemovalFailed(
                        this.accountName,
                        this.taskHub,
                        this.workerName,
                        lease.PartitionId,
        ...
        async Task RemoveLeaseAsync(T lease, bool hasOwnership)
        {
            ...
            {
                EtwEventSource.Log.PartitionRemoved(
                    this.accountName,
                    this.taskHub,
                    this.workerName,
                    lease.PartitionId,
