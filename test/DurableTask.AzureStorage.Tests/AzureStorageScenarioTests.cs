...
namespace DurableTask.AzureStorage.Tests
{
    ...
    public class AzureStorageScenarioTests
    {
        ...
        /// <summary>
        /// Test the message encoding style for Azure Storage queues is consistent with the one specified when initializing StorageAccountClientProvider.
        /// </summary>
        [TestMethod]
        [DataRow(null)]
        [DataRow(QueueMessageEncoding.Base64)]
        public async Task TestQueueMessageEncodingSeetings(QueueMessageEncoding? queueMessageEncoding = null)
        {
            ...
            {
                PartitionCount = 1,
                StorageAccountClientProvider = new StorageAccountClientProvider(connectionString, queueMessageEncoding),
                TaskHubName = "TestQueueMessageEncoding",
            };
            
            var service = new AzureStorageOrchestrationService(settings);
            var client = new TaskHubClient(service);

            // Create a queue client with the WindowsAzure.Storage v9.x SDK since that's what's used by DTFx.AzureStorage v1.x
            CloudStorageAccount storageAccount = CloudStorageAccount.Parse(connectionString);
            CloudQueueClient queueClient = storageAccount.CreateCloudQueueClient();
            CloudQueue queue = queueClient.GetQueueReference("testqueuemessageencoding-control-00");

            // Add a orchestration message to the control queue
            await client.CreateOrchestrationInstanceAsync(typeof(Orchestrations.LongRunningOrchestrator), input: "0");
            var message = await queue.GetMessageAsync();

            try
            {
                // If the message can be successfully read, queueMessageEncoding should be QueueMessageEncoding.Base64
                var content = message.AsString;
                
                Assert.AreEqual(QueueMessageEncoding.Base64, queueMessageEncoding);
                Assert.IsNotNull(message.AsString);
                StringAssert.Contains(message.AsString, "DurableTask.Core.History.ExecutionStartedEvent");
            }
            catch(FormatException)
            {
                // If the message can't be read, queueMessageEncoding should be QueueMessageEncoding.None
                Assert.IsNull(queueMessageEncoding);
            }
